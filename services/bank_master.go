...
func (s *bankMasterService) Create(ctx context.Context, companyCode string, req *requests.CreateBankMasterRequest) (*models.BankMaster, error) {
	...
	if bank.IsDefault {
		// Unset default for all other banks in this company
		_, _ = collection.UpdateMany(ctx, bson.M{}, bson.M{"$set": bson.M{"is_default": false}})
	}
...
func (s *bankMasterService) Update(ctx context.Context, companyCode string, id string, req *requests.UpdateBankMasterRequest) (*models.BankMaster, error) {
	...
	if req.IsDefault != nil {
		...
		if *req.IsDefault {
			// Unset default for all other banks
			_, _ = collection.UpdateMany(ctx, bson.M{"entity_id": bson.M{"$ne": id}}, bson.M{"$set": bson.M{"is_default": false}})
		}
	}
